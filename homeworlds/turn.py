    ...
    def addEvent(self,e):
        ...
        if self.isSac is None:
            ...
            if isinstance(e,event.Pass)\
              or isinstance(e,event.Creation):
                # Not a sacrifice
                self.isSac = False
            ...
            # This is not a sacrifice, so only a Fade or Catastrophe should be accepted
            if not (
                isinstance(e,event.Catastrophe) or
                isinstance(e,event.Fade) or
                isinstance(e,event.Elimination)
            ):
                print 'tried to do more'
                print self
                print e
    ...
    def undoLast(self):
        ...
        if self.isSac:
            if isinstance(last,event.Sacrifice):
                ...
                self.colSac = None
                self.nSac = None
            elif(isinstance(last,event.Action)
            ...
            if (isinstance(last,event.Action)
                ...
                # it previously proved this was not a sacrifice turn
                self.isSac = None
